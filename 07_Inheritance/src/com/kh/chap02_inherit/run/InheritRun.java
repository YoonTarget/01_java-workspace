...
public class InheritRun /*extends Object*/ {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * * 상속의 특징
		 * - 클래스 간의 상속에 있어서는 다중상속이 안된다!!! (단일상속만 가능. 즉, 부모는 하나다)
		 * - 자식 객체는 부모 클래스에 있는 메소드를 마치 내 것처럼 호출 가능
		 * 		+ 부모 클래스에 있는 메소드가 마음에 안 들면 자식 클래스에서 오버라이딩 가능 (내 입맛대로 재정의)
		 * - 부모 클래스에 정의되어 있는 protected 필드는 자식 클래스에서 직접 접근 가능
		 * - 명시되어 있지는 않지만 모든 클래스 (제공하는 클래스, 직접 만든 클래스)는 Object 클래스의 후손이다.
		 * 		=> 즉, Object 클래스에 있는 메소드들 마음대로 호출 가능!!
		 * 		=> 뿐만 아니라 오버라이딩을 통해 재정의도 가능함!! 
		 */
		
		System.out.println(c.hashCode());
		System.out.println(c.equals(s)); // 그럼 부모가 2명인가요?? // Object는 모든 클래스의 조상!!
		System.out.println(c/*.toString()*/); // 중요!! toString이 자동으로 나왔던 것!!
