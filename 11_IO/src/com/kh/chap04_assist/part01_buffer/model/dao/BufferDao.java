...
public class BufferDao {
	...
	/*
	 * 보조 스트림 : 기반 스트림만으로 부족했던 성능을 보다 향상 시켜주는 스트림
	 * 				기반 스트림에서 제공하지 않는 추가적인 메소드 제공 / 데이터 전송 속도 향상 시킴 + a
	...
	public void fileSave() {
		// FileWriter 기반 스트림 가지고 해보자!! (기반 스트림 필수)
		// 파일과 직접적으로 연결해서 2바이트 단위로 출력할 수 있는 스트림
		
		// BufferedWriter : 버퍼라는 공간을 제공해주는 보조 스트림 (속도 향상) : 기반 스트림이랑 이름 맞춰야 함
		...
		*/
		...
		try (BufferedWriter bw = new BufferedWriter(new FileWriter("c_buffer.txt"))) {
			...
			e.printStackTrace();
		} // 자원 반납 구문 안 써도 됨!! 자동으로 반납까지 해줄거임!!
		
	}
	
	// 프로그램 <----- 파일(입력)
	public void fileRead() {
		// FileReader : 파일과 연결해서 2바이트 단위로 데이터를 입력 받을 수 있는 기반 스트림
