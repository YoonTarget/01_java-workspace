...
public class FileRun {
	...
	public static void main(String[] args) {
		...
		try {
			...
			// 실행할 때마다 파일이 계속 만들어지나요??
			// 해당 파일이 존재하면 그냥 덮어쓰기 된다. 수정사항 없으면 그대로 감
			...
			System.out.println("상위폴더 : " + folder.getParent());
	...
	/*
	 * 프로그램 상의 데이터를 외부 매체로 출력을 한다거나 외부 매체로부터 입력을 받아올거임~!~!
	 * 반드시 그 외부매체와 통하는 통로를 만들어야 됨!! 그게 바로 스트림(개울, 흐름)!!
	 * 
	 * 스트림(통로)의 특징
	 * - 스트림 : 개울 / 한 방향으로 흐름
	 * - 단방향 : 입력이면 입력만 / 출력이면 출력만 가능!!
	 * 			=> 입력과 출력을 동시다발적으로 하고자 한다면?? 입력용 스트림 / 출력용 스트림 따로 열어야 됨!!
	 * - 선입선출(FIFO) : 먼저 들어간 데이터가 먼저 나온다. // 통로가 파이프 같은 개념
	 * - 시간 지연(Delay) : 이라는 문제가 생길 수 있음!!
	 * 
	 * * 스트림의 구분
	 ...
	 * 				보조 스트림 단독으로는 사용 불가!! 기반 스트림은 반드시 기본적으로 있어야 됨!!
	 * 
	 * 우리는 외부매체를 파일로 해보자!!
