...
public class ServerProgram {
	...
	public static void main(String[] args) {
		...
		// 1) 포트 번호 지정 (서버 측에서 몇 번 포트로 통로를 열거냐)
		int port = 3000;
		...
		try {
			...
			// 4) 연결 요청이 오면 요청 수락 후 해당 클라이언트와 통신할 수 있는 서버 측 소켓 객체 생성
			Socket socket = server.accept(); // socket == 클라이언트와 통신할 수 있는 소켓 객체
			System.out.println(socket.getInetAddress().getHostAddress() + "가 연결을 요청함...");
			
			// 5) 클라이언트와 입출력 스트림 생성 (바이트 스트림으로밖에 안됨!! => 1byte짜리!!) => 성능이 별로 안 좋음
			
			// 6) 보조 스트림을 통해서 성능 개선
			// 	  입력용 스트림 => 1바이트 짜리 바이트 스트림을 문자 스트림으로 변경해줌!! => 성능 더 좋은 보조 스트림 껴서
			...
			while(true) {
				// 7) 스트림을 통해 읽고 쓰기
				// 클라이언트로부터 전달된 메세지가 있을 경우 서버 측에서 읽어들일거임!! (입력)
