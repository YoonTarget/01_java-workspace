...
public class EasyRun {
	...
	public static void main(String[] args) {
		...
		Sports[] arr = new Sports[2]; // 배열만 만든 거
		// 부모			자식
		...
		/*
		 * 동적 바인딩 : 컴파일 시점에서는 정적 바인딩으로 현재 레퍼런스의 자료형의 클래스에 있는 메소드를 가리키긴 함
		 * 				런타임 시점(실행시점)에서는 동적 바인딩으로 각각의 자식 클래스에 오버라이딩 된 메소드가 있을 경우 그거 실행
		 */
		 ...
		 * - 미완성된 클래스
		 * - 일반필드 + 일반메소드 [+ 추상메소드]
		 * => 추상메소드를 가진 클래스라면 반드시 추상클래스로 명시 해야됨
		 * 
		 * * 추상메소드가 굳이 없어서 추상클래스로 만들 수 있음
		 * 	 언제 사용되나요?
		 * 
